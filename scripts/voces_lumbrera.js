...
function listar_obras(){
	...
	$.post("ajax/voces_lumbrera.php?op=listar_obras&nombre="+nombre,function(r){
		$("#box_obras").html(r);
	});
}
...
function listar_voces(idobra, nombre){
	$.post("ajax/voces_lumbrera.php?op=listar_voces&idobra="+idobra+"&nombre="+nombre,function(r){
		$("#box_voces").html(r);
...
function listar_obras_1(){
	$.post("ajax/voces_lumbrera.php?op=listar_obras_1",function(r){
		$("#box_obras").html(r);
        document.getElementById("div_busquedas").style.display = "block";
		document.getElementById("div_voces").style.display = "none";
	});
